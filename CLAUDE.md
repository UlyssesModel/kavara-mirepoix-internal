...
- **Negative type-smokes must assert exit code AND diagnostic-anchored content.** Every CI step that exercises a negative type-smoke (a `tsconfig-negative.json` whose smoke files are designed to fail `tsc`) MUST run through [`scripts/check-negative-smoke.sh`](scripts/check-negative-smoke.sh) or an equivalent runner that captures `tsc` output and asserts both (a) tsc exited non-zero, AND (b) the expected diagnostic patterns are present in the output. **Patterns must be diagnostic-anchored** — bare-word patterns (e.g., just `'dispatchId'`) match anywhere in tsc's output and false-positive when the same word appears in unrelated expected-type renderings. The runner **structurally enforces** anchoring: each pattern MUST contain at least one of `\.ts` (file reference), `TS[0-9]+` (TypeScript error code), `Property '` ("Property 'X' is missing" clause start), `missing the following properties` (multi-field missing clause), or `\([0-9]+,[0-9]+\)` (line:col location). Zero-pattern and bare-word/catch-all invocations are rejected with exit 2. CI also runs self-tests verifying the runner's guards still fire. **Bare `! bun x tsc -p ...` is forbidden** for the same reason at the gate layer. Origins: Codex adversarial-review on PR #20 (bare-`! tsc`, 2026-05-20), PR #30 round 1 (bare-word patterns, 2026-05-20), PR #30 round 2 (zero-pattern invocations, 2026-05-20), PR #30 round 3 (runner-enforced bare-word rejection, 2026-05-20).
...
- Wire a bare `! bun x tsc -p ...tsconfig-negative.json` into CI for a negative type-smoke — it succeeds on any non-zero tsc exit and silently masks drift where the smoke stops compiling for the wrong reason. Use [`scripts/check-negative-smoke.sh`](scripts/check-negative-smoke.sh) (or an equivalent that asserts both exit-code AND diagnostic content) instead. Codex adversarial-review on PR #20 (2026-05-20) is the origin.
- Use bare-word patterns (e.g., just `'dispatchId'`) when configuring [`scripts/check-negative-smoke.sh`](scripts/check-negative-smoke.sh) — bare words match anywhere in `tsc`'s output, including unrelated expected-type renderings where the same name appears as a satisfied property. Anchor each pattern to its diagnostic (`"Property 'X' is missing"`, `"missing the following properties.*X"`, or `file.ts(line,col): error TS####`). Codex adversarial-review on PR #30 (2026-05-20) is the origin.

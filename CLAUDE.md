...
- **Negative type-smokes must assert exit code AND diagnostic content.** Every CI step that exercises a negative type-smoke (a `tsconfig-negative.json` whose smoke files are designed to fail `tsc`) MUST run through [`scripts/check-negative-smoke.sh`](scripts/check-negative-smoke.sh) or an equivalent runner that captures `tsc` output and asserts both (a) tsc exited non-zero, AND (b) the expected diagnostic patterns (TypeScript error codes, smoke filename, the specific field name or bad-tag string the smoke types against) are present in the output. **Bare `! bun x tsc -p ...` is forbidden** — it accepts any non-zero exit and silently masks drift where the smoke stops compiling for an unrelated reason (renamed import, module-resolution break). Origin: Codex adversarial-review finding on PR #20, 2026-05-20.
